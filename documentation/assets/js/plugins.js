...
function numericStepper(){
    if ($('.numeric-stepper').length && $.fn.TouchSpin) {
        $('.numeric-stepper').each(function () {
            if($(this).data('decimal')) alert('ooi');
            $(this).TouchSpin({
       ...
       setTimeout(function () {
            $('select').each(function(){
                function format(state) {
                    ...
                    if(res[0] == 'image') {
                        if(res[2]) return "<img class='flag' src='assets/images/flags/" + res[1].toLowerCase() + "-" + res[2].toLowerCase() +".png' style='width:27px;padding-right:10px'/>" + state.text;
                        else return "<img class='flag' src='assets/images/flags/" + res[1].toLowerCase() + ".png' style='width:27px;padding-right:10px'/>" + state.text;
                    }
                    else {
                        return state.text; 
                    }
    ...
    if ($('.slider').length && $.fn.slick) {
        $('.slider').each(function () {
            $(this).slick({        
                ...
                autoplaySpeed: 4000, // Auto play change interval
